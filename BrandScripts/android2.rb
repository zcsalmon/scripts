...
def grabColors(brand)
  returnDir = Dir.pwd
  targetDir = "#{$MyPath}/mpp/media/#{brand}/android2/"
  if File.exists?(targetDir) then
    Dir.chdir(targetDir)
    File.open('client.config', 'r') do |output|
...
def fileInput(brand, color)
  ...
  if File.exists?(targetDir) then
    ...
    File.open('client.config','a') do |input|
      ...
      input.puts("color_selection_background=" + adjustToWhite(color).to_str) #$globalSelectedHash[brand].sub(/#/, ""))
